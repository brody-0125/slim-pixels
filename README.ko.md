...
[English](https://github.com/brody-0125/slim-pixels/blob/main/README.md) | 한국어

Dart에서 이미지의 크기와 형태를 변환하고 PNG, JPEG, 무손실 WebP로 인코딩합니다. 인코딩된 바이트와 타입이 있는 작업 목록을 받아, 읽기 전용 바이트와 출력 메타데이터를 반환합니다.

## 주요 기능

- **플랫폼**: Dart 3.10.0 이상, 4.0.0 미만. Windows/Linux x64 네이티브 자산을 포함합니다.
- **입력 포맷**: 정적 PNG/JPEG/WebP에서 디코딩한 RGB8/RGBA8.
- **변환**: exact/inside/cover resize, crop, 90도 단위 회전, 좌우·상하 반전.
- **출력 포맷**: JPEG, PNG, 무손실 WebP.
- **Worker 처리**: 단일 isolate, 제한된 FIFO 큐, 수락한 작업을 완료한 뒤 종료.
...
0.1.3이 pub.dev에 게시되면 다음 의존성을 추가합니다. GitHub 태그 `v0.1.3`은 만들었고, pub.dev 게시는 아직입니다.
...
| API | 반환값 | 용도 |
|---|---|---|
| `SlimPixels()` | `SlimPixels` | 동기 호출용 네이티브 자산 초기화. |
| `transformSync(input, operations:, encoding:)` | `ImageResult` | 호출자 isolate에서 디코딩·변환·인코딩. |
| `SlimPixelsWorker.start(...)` | `Future<SlimPixelsWorker>` | isolate 생성 후 네이티브 초기화까지 대기. |
| `worker.transform(input, operations:, encoding:)` | `Future<ImageResult>` | worker 용량 한도 내에서 요청 제출. |
| `worker.close()` | `Future<void>` | 수락한 작업 처리 후 종료까지 대기. |

`ImageResult`의 `bytes`, `width`, `height`, `format`, `mimeType`, `byteLength`로 인코딩 결과와 메타데이터를 읽습니다. 두 변환 메서드 모두 `encoding`이 필수입니다.
...
| `Resize.inside(maxWidth:, maxHeight:)` | 지정한 경계 안에 종횡비를 유지해 맞춥니다. 최소 한 경계를 지정해야 합니다. 기본적으로 확대하지 않습니다. |
...
- 입력은 1바이트..256 MiB, 작업은 최대 64개입니다. 입력 한 축은 최대 16,384픽셀, 입력·중간·출력 이미지는 최대 3,200만 픽셀입니다. 디코더 할당 제한은 256 MiB이며 프로세스 전체 메모리 상한은 아닙니다.
...
Dart SDK와 Python을 준비한 뒤 저장소 루트에서 실행합니다.
...
Worker AOT와 선정한 회귀 검사는 다음과 같이 실행합니다.
...
`python` 명령이 없는 환경에서는 `python3`를 사용하세요. CI는 Windows/Linux의 지원 Dart Stable 패치 버전을 검사하며, Dart 3.10.0 최소 의존성 작업을 별도로 실행합니다. Golden 검사는 인코딩된 바이트와 거부 정책을 확인하고, 네이티브 메모리 검사는 Linux harness와 Valgrind를 사용합니다.
...
[MIT](https://github.com/brody-0125/slim-pixels/blob/main/LICENSE), Copyright (c) 2026 Seokhyeon Kim. 바이너리를 배포할 때 [제3자 고지](https://github.com/brody-0125/slim-pixels/blob/main/THIRD_PARTY_NOTICES.md)와 `third_party/`를 함께 포함하세요.
